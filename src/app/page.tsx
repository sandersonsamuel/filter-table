    ...
    <>
      <div className={"flex flex-col w-screen h-screen p-10"}>
        <div className={"flex flex-col items-center"}>
          <form onSubmit={defineParams} className={"flex items-end w-1/3 gap-5"}>

            <div className={"flex flex-col"}>
              <label htmlFor="id">Filtrar por id:</label>
              <Input value={filters.id} onChange={(e) => setFilters({...filters, id: String(e.target.value)})}
                     type={"text"} id={"id"} placeholder={"id"}/>
            </div>

            <div className={"flex flex-col"}>
              <label htmlFor="nome">Filtrar por nome:</label>
              <Input value={filters.name} onChange={(e) => setFilters({...filters, name: String(e.target.value)})}
